...
duration<int, milli> random_time(const size_t max_delay) {
  srand(time(NULL));
  duration<int, milli> result(rand() % (max_delay + 1));
  return result;
}

void TestParallelSearch(vector<pair<istringstream, ostringstream*>>& streams, const size_t max_delay) {
  LOG_DURATION("Total");
  SearchServer srv(streams.front().first);
  for (auto& [input, output] :
       IteratorRange(begin(streams) + 1, end(streams))) {
    this_thread::sleep_for(random_time(max_delay));
    if (!output) {
...
void TestFunctionality(
  ...
  streams.push_back(make_pair(move(docs_input), nullptr));
  streams.push_back(make_pair(move(queries_input), &queries_output));
  TestParallelSearch(streams, 0);
